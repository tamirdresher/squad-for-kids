...
1. **🚨 Blockers & Critical Items** — Issues with blocker/urgent/sev/incident labels
2. **🔄 Open Pull Requests** — All open PRs with author and age
3. **📋 Key Open Issues** — Grouped by label
4. **📈 Yesterday's Activity** — Commits and closed PRs from last 24h
5. **📄 Loop Doc Status** — Placeholder (manual check link)
6. **💬 Recent RP Discussions** — Placeholder (requires WorkIQ MCP)
7. **📅 Today's Meetings** — Placeholder (requires WorkIQ MCP)
8. **✅ Action Items** — Old PRs needing review, blockers requiring attention
...
The script may show warnings for closed PR date parsing — this is non-fatal and doesn't affect the briefing.

## Related Files

- `.squad/scripts/daily-rp-briefing.ps1` — Main briefing script
- `ralph-watch.ps1` — Ralph watch loop with scheduling integration
- `$env:USERPROFILE\.squad\teams-webhook.url` — Teams webhook URL
- `$env:USERPROFILE\.squad\ralph-watch.log` — Ralph execution logs
