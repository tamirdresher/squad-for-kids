...
**Git worktrees are mandatory.** Never `git checkout` or `git switch` branches in the main working directory. Always use `git worktree add` to create an isolated directory for branch work. See `.squad/routing.md` → "Git Worktree Convention" for naming and cleanup rules. This prevents conflicts when multiple agents run in parallel.
