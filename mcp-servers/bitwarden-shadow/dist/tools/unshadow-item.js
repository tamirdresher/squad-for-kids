...
export async function unshadowItem(config, args) {
    try {
        ...
        if (remaining.length === 0)
            return err(`Refusing: removing "${collection.name}" would leave "${item.name}" with no collections.`);
        ...
        return err(`unshadow_item failed: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
//# sourceMappingURL=unshadow-item.js.map