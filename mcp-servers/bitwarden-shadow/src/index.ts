...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
      case "shadow_item":   return await shadowItem(config, (args ?? {}) as { item_id: string; target_collection?: string });
      case "unshadow_item": return await unshadowItem(config, (args ?? {}) as { item_id: string; target_collection?: string });
      case "list_shadows":  return await listShadows(config, (args ?? {}) as { collection?: string; include_available?: boolean });
  ...
  if (config.adminCollectionId) console.error(`Admin collection:  ${config.adminCollectionId}`);
}
