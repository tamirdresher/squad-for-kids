...
**Example:** Outlook COM automation fails → fall back to Playwright browser automation for the same email task.
...
**Example:** Build fails with CS0246 "type not found" → check for missing using/import → add it → rebuild.
...
**Example:** After 3 failed build attempts → "Build fails on line 42 with null reference. Tried X, Y, Z. Likely a design issue in the Foo module. Recommend Data or the code owner review."
