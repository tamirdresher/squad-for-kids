...
metadata:
  version: 1.0.0-squad
  adapted_from: MDE.ServiceModernization.CopilotCliAssets (rimuri/reflect)
---

# Reflect Skill

**Critical learning capture system** adapted for Squad architecture. Prevents repeating mistakes and preserves successful patterns across sessions.
...
2. **`.squad/decisions.md`** — Team-wide decisions that all agents respect (managed by Scribe)
...
- Promote HIGH confidence patterns → Picard/Scribe for decision.md review
...
1. **Agent-specific learning** → `.squad/agents/{agent}/history.md` (via agent self-update or Picard review)
2. **Team-wide decision** → `.squad/decisions/inbox/{agent}-{topic}.md` (for Scribe review)
...
**Example:**
```text
User consistently uses git --no-pager
→ [LOW] ~ Note for review: "User prefers --no-pager for git commands"
```
...
│   Target: .squad/agents/picard/history.md                   │
│                                                             │
...
2. **For Team Decisions:**
   ...
   - Scribe reviews and merges to `decisions.md` if appropriate
   - Format follows Decision template (see decisions.md)

3. **For Skills:**
   - Document recommendation in session notes
   - Squad lead (Picard) reviews and routes to skill owner

**Memory Format Example (Agent History):**

```markdown
### {Date}: {Agent} — {Assignment Title}
...
[MED] + Add preference: "Include test plan in PR descriptions"
  ...
  Target: .squad/decisions/inbox/picard-pr-format.md (for team adoption)
```
...
**Differences from MDE version:**
- Adapted storage paths to `.squad/` structure
- Routes team-wide learnings through `.squad/decisions/inbox/` for Scribe review
- Agent-specific learnings append to `{agent}/history.md`
- No Serena MCP dependency (Squad uses Git-based storage)
