...
description: "Max-3-cycle protocol for agent sub-tasks with WHY context and coordinator validation"
...
Adopted from the everything-claude-code audit (issue #1295).
...
1. **After each cycle**, the coordinator (Ralph) evaluates the output against the success criteria
...
Before accepting agent output and closing an issue, Ralph must check:
...
```
## Task
Add an "Iterative Retrieval Protocol" section to `.squad/agents/ralph/charter.md` explaining
the 3-cycle rule, WHY format, and validation checklist.

## WHY this matters
Ralph spawns sub-agents on every round. Without a documented protocol, agents run unbounded
cycles and outputs go unvalidated — leading to stale issues and silent failures. This section
makes the protocol discoverable by any future Ralph instance or squad member.
...
## Escalation path
If the charter.md format is unclear, check .squad/agents/picard/charter.md as a reference.
If uncertain about content, stop and surface to coordinator.
```

### Bad spawn prompt (don't do this)
```
Update ralph's charter with the iterative retrieval stuff from issue 1317.
```
