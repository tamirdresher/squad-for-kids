...
.DESCRIPTION
    ...
    If `agency` is not found in PATH, the script prints the generated
    prompt so you can paste it into your Copilot CLI manually.

.PARAMETER Hours
    How many hours back to search for sessions. Default: 24.

.PARAMETER Filter
    Optional keyword to search for in session content (e.g., "auth",
    "deployment", "refactor"). Uses FTS5 search when provided.

.PARAMETER ExcludeRalph
    Exclude Ralph/keep-alive/heartbeat monitoring sessions from results.
    Default: $true. Set -ExcludeRalph:$false to include them.
...
.EXAMPLE
    # Find sessions from the last 24 hours, excluding monitoring agents
    .\recover-sessions.ps1

    # Look back 48 hours, filter for "auth"
    .\recover-sessions.ps1 -Hours 48 -Filter "auth"

    # Find sessions for a specific project folder
    .\recover-sessions.ps1 -WorkingDir "my-project"

    # Just list, don't prompt to resume
    .\recover-sessions.ps1 -ListOnly

    # Include Ralph/monitoring sessions
    .\recover-sessions.ps1 -ExcludeRalph:$false
#>
[CmdletBinding()]
param(
    [int]$Hours = 24,
