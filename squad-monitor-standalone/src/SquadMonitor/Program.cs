...
var configDir = ".squad"; // configurable via --config-dir
...
var teamRoot = FindTeamRoot(configDir);

if (teamRoot == null)
{
    AnsiConsole.MarkupLine($"[red]Error: Could not find '{configDir}' directory. Run from project root or use --config-dir.[/]");
    return 1;
}

// Initialize agent log parser for live activity tracking
var agentLogParser = new AgentLogParser();

AnsiConsole.MarkupLine($"[dim]Squad Monitor v2 - Refresh interval: {interval}s[/]");
AnsiConsole.MarkupLine($"[dim]Project root: {teamRoot}[/]");
...
if (runOnce)
{
    ...
    DisplayRalphHeartbeat(userProfile, configDir);
    DisplayRalphLog(userProfile, configDir);
    DisplayGitHubIssues(teamRoot);
    DisplayGitHubPRs(teamRoot);
    DisplayRecentlyMergedPRs(teamRoot);
    var activities = LoadActivities(teamRoot, configDir);
    DisplayOrchestrationLog(activities);

    // Live Agent Activity (run-once mode)
    agentLogParser.ParseLatestLogs();
    DisplayAgentActivity(agentLogParser);
}
else
{
    ...
    await AnsiConsole.Live(layout)
        ...
        .StartAsync(async ctx =>
        {
            do
            {
                ...
                // Refresh agent logs each cycle
                agentLogParser.ParseLatestLogs();

                var now = DateTime.Now;
                var content = orchestrationOnlyMode
                    ? BuildOrchestrationOnlyContent(now, userProfile, teamRoot, configDir, agentLogParser)
                    : BuildDashboardContent(now, userProfile, teamRoot, configDir, agentLogParser);
...
static IRenderable BuildDashboardContent(DateTime now, string userProfile, string teamRoot, string configDir, AgentLogParser agentLogParser)
{
    var sections = new List<IRenderable>();

    int termHeight = 50;
    try { termHeight = Console.WindowHeight; } catch { }
    int reservedLines = 38; // extra room for agent activity section
    ...
    sections.Add(BuildRalphHeartbeatSection(userProfile, configDir));
    sections.Add(BuildRalphLogSection(userProfile, configDir));
    ...
    // Live Agent Activity
    sections.Add(BuildAgentActivitySection(agentLogParser));
...
static IRenderable BuildOrchestrationOnlyContent(DateTime now, string userProfile, string teamRoot, string configDir, AgentLogParser agentLogParser)
{
    var sections = new List<IRenderable>();
    ...
    // Live Agent Activity (also shown in orchestration view)
    sections.Add(BuildAgentActivitySection(agentLogParser));
...
static string? FindTeamRoot(string configDir)
{
    var current = Directory.GetCurrentDirectory();
    while (current != null)
    {
        if (Directory.Exists(Path.Combine(current, configDir)))
            return current;
...
static IRenderable BuildAgentActivitySection(AgentLogParser parser)
{
    var items = new List<IRenderable>();

    var section = new Rule("[green]Live Agent Activity[/]") { Justification = Justify.Left };
    items.Add(section);

    var entries = parser.GetRecentEntries();
    if (entries.Count == 0)
    {
        items.Add(new Markup("[dim]  No agent activity detected yet — waiting for logs[/]"));
    ...
    var table = new Table()
        ...
        .AddColumn(new TableColumn("Agent").Width(12))
        .AddColumn(new TableColumn("Message").Width(50))
        .AddColumn(new TableColumn("Age").Width(10));

    foreach (var entry in recent)
    {
        var age = FormatAge(DateTime.Now - entry.Timestamp);
        ...
        table.AddRow(
            $"[{typeColor}]{Markup.Escape(entry.Type)}[/]",
            $"[white]{Markup.Escape(entry.Agent)}[/]",
    ...
    items.Add(table);
    items.Add(new Markup($"[dim]  Showing {recent.Count} of {entries.Count} total entries[/]"));
    items.Add(Text.Empty);
    return new Rows(items);
}

static void DisplayAgentActivity(AgentLogParser parser)
{
    var section = new Rule("[green]Live Agent Activity[/]") { Justification = Justify.Left };
    ...
    if (entries.Count == 0)
    {
        AnsiConsole.MarkupLine("[dim]  No agent activity detected yet — waiting for logs[/]");
        AnsiConsole.WriteLine();
        return;
    }
    ...
    table.AddColumn(new TableColumn("[bold]Agent[/]").LeftAligned());
    table.AddColumn(new TableColumn("[bold]Message[/]").LeftAligned());
    table.AddColumn(new TableColumn("[bold]Age[/]").RightAligned());
    ...
    AnsiConsole.MarkupLine($"[dim]  Showing {recent.Count} of {entries.Count} total entries[/]");
    AnsiConsole.WriteLine();
}

// ─── Section Builders (return IRenderable) ──────────────────────────────────

static IRenderable BuildRalphHeartbeatSection(string userProfile, string configDir)
{
    var items = new List<IRenderable>();

    var section = new Rule("[cyan]Automation Watch Loop[/]") { Justification = Justify.Left };
    items.Add(section);

    var heartbeatPath = Path.Combine(userProfile, configDir, "ralph-heartbeat.json");
    if (!File.Exists(heartbeatPath))
    {
        items.Add(new Markup("[dim]  No heartbeat file found — automation watcher may not be running[/]"));
        items.Add(Text.Empty);
        return new Rows(items);
    }
...
static IRenderable BuildRalphLogSection(string userProfile, string configDir)
{
    var items = new List<IRenderable>();

    var logPath = Path.Combine(userProfile, configDir, "ralph-watch.log");
    if (!File.Exists(logPath))
    {
        return Text.Empty;
    }

    var section = new Rule("[cyan]Recent Automation Rounds[/]") { Justification = Justify.Left };
    items.Add(section);
    ...
    try
    {
        ...
        foreach (var line in last5)
        {
            ...
            if (match.Success)
            {
                ...
                {
                    var color = GetLogLineColor(trimmed);
    ...
    {
        items.Add(new Markup("[red]  Error reading watch log[/]"));
    }
...
static string GetLogLineColor(string line)
{
    if (line.Contains("✓")) return "green";
    if (line.Contains("→")) return "cyan";
    if (line.Contains("⚠") || line.Contains("WARN")) return "yellow";
...
// ─── Run-Once Display Methods ───────────────────────────────────────────────

static void DisplayRalphHeartbeat(string userProfile, string configDir)
{
    var section = new Rule("[cyan]Automation Watch Loop[/]") { Justification = Justify.Left };
    AnsiConsole.Write(section);
    ...
    if (!File.Exists(heartbeatPath))
    {
        AnsiConsole.MarkupLine("[dim]  No heartbeat file found — automation watcher may not be running[/]");
        AnsiConsole.WriteLine();
        return;
    }
...
static void DisplayRalphLog(string userProfile, string configDir)
{
    var logPath = Path.Combine(userProfile, configDir, "ralph-watch.log");
    if (!File.Exists(logPath))
    {
    ...
    {
        AnsiConsole.MarkupLine("[red]  Error reading watch log[/]");
    }
...
// ─── Orchestration Log Loader ───────────────────────────────────────────────

static List<AgentActivity> LoadActivities(string teamRoot, string configDir)
{
    var activities = new List<AgentActivity>();
    var orchestrationLogPath = Path.Combine(teamRoot, configDir, "orchestration-log");
