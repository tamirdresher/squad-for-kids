...
- You need to find a past session by topic (e.g., "auth", "deployment", "refactor")
...
### 2. Filter Out Monitoring Agent Sessions

Monitoring agents (e.g., Ralph, keep-alive bots) create many sessions. Exclude them to reduce noise:
...
> **Tip:** If your deployment uses a different monitoring agent name, add it to the `LIKE` clauses above.
...
- **FTS5 query expansion**: The `search_index` table uses keyword matching, not semantic search. Always search for synonyms: `'auth OR login OR token OR JWT'`
...
- Don't skip the monitoring agent filter — automated sessions are high-volume and will flood results
