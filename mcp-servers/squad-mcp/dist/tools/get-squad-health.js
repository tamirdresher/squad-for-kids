...
export async function getSquadHealth(config, args) {
    try {
        const includeMetrics = args.includeMetrics ?? true;
        const githubClient = new GitHubClient(config);
        const squadState = new SquadState(config);
        // Gather data
        const [teamMembers, openIssuesCount, openPRsCount, allIssues, untriagedIssues, copilotIssues, lastBoardUpdate,] = await Promise.all([
        ...
        // Calculate metrics
        const teamSize = teamMembers.filter((m) => m.status.includes("✅") || m.status.includes("Active")).length;
        ...
        const memberStats = await Promise.all(teamMembers.map(async (member) => {
...
//# sourceMappingURL=get-squad-health.js.map